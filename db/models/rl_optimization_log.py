...
class RlOptimizationLog(Base):
    ...
    # 실험 메타데이터
    dataset_size = Column(Integer, nullable=True)  # 실험에 사용된 Train 데이터 개수
    train_batch_size = Column(Integer, nullable=True)  # 학습 시 iteration당 Train batch 크기
    avg_total_score = Column(Float, nullable=True)  # 해당 에피소드의 Train 과정 중 Validation 평균 점수
    ...
    # Test 관련 컬럼
    test_info = Column(JSONB, nullable=True)  # Test 샘플들의 상세 정보 (JSON 형태)
    test_accuracy = Column(Float, nullable=True)  # Test 데이터셋 전체의 평균 accuracy
    test_dataset_size = Column(Integer, nullable=True)  # Test 데이터셋 샘플 개수
    ...
    # Backward Judge (비평가 엔진) 관련 정보
    evaluation_instruction = Column(Text, nullable=True)  # 비평가 엔진에게 전달하는 평가 기준 (우선순위, 고려사항 등)
    backward_judge_total_input = Column(Text, nullable=True)  # 비평가 엔진에게 전달되는 전체 입력 내용 (추정값)
