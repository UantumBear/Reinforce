"""
@경로: scripts/data_prep/download_finance_law_merge_rag.py
@설명: HuggingFace의 Finance Law Merge RAG 데이터셋을 로드하고 샘플 데이터를 저장/확인할 수 있는 클래스
@명령어: python scripts/data_prep/download_finance_law_merge_rag.py
...
class FinanceLawMergeRagDataset:
    """Finance Law Merge RAG 데이터셋 관리 클래스"""
    
    def __init__(self, split="train"):
        """
        Finance Law Merge RAG 데이터셋 로드
        split: 'train' (학습용), 'validation' (평가용), 'test' (테스트용)
        """
        # HuggingFace 다운로드 경로 정보 로깅
        self.dataset_name = "HJUNN/Finance-Law-merge-rag-dataset"

        logger.info(f"Loading {self.dataset_name} dataset ({split})...")
        logger.info(f"Dataset identifier: {self.dataset_name}")
        logger.info(f"HuggingFace URL: https://huggingface.co/datasets/{self.dataset_name}")
        ...
        try:
            ...
            logger.info(f"[CHECK] {self.dataset_name} dataset loaded successfully. Size: {len(self.dataset)}")
    ...
    def save_full_dataset(self, output_format: str = "json") -> str:
        """
        전체 데이터셋을 datafile/original/ 경로에 저장
        
        Args:
            output_format (str): "json", "csv", "parquet", "arrow" 중 선택
        ...
        # datafile/original/HJUNN 폴더 구조 생성
        dataset_parts = self.dataset_name.split("/")
        if len(dataset_parts) == 2:
            author, dataset_name = dataset_parts
            output_dir = Path("datafile/original") / author
        else:
            # fallback - 단일 이름인 경우
            dataset_name = self.dataset_name.replace("/", "_")
            output_dir = Path("datafile/original")
        
        output_dir.mkdir(parents=True, exist_ok=True)
        
        # 파일명 생성 (하이픈을 언더스코어로)
        safe_filename = dataset_name.replace("-", "_")
        
        logger.info(f"Saving full dataset ({len(self.dataset)} samples) to {output_dir}/...")
        
        if output_format.lower() == "json":
            output_file = output_dir / f"{safe_filename}_full.json"
            self.dataset.to_json(str(output_file), orient="records", lines=False, force_ascii=False)
            
        elif output_format.lower() == "csv":
            output_file = output_dir / f"{safe_filename}_full.csv"
            self.dataset.to_csv(str(output_file), index=False)
            
        elif output_format.lower() == "parquet":
            output_file = output_dir / f"{safe_filename}_full.parquet"
            self.dataset.to_parquet(str(output_file))
            
        elif output_format.lower() == "arrow":
            output_file = output_dir / f"{safe_filename}_full"
            self.dataset.save_to_disk(str(output_file))
            
        else:
            raise ValueError("output_format must be 'json', 'csv', 'parquet', or 'arrow'")
        
        logger.info(f"Full dataset saved to: {output_file}")
        
        # 파일 크기 정보 로깅
        if output_format != "arrow":  # arrow는 폴더이므로 파일 크기 체크 안함
...
if __name__ == "__main__":
    ...
    사용법: uv run python scripts/data_prep/download_finance_law_merge_rag.py
    """
    try:
        print(f"데이터셋 샘플 저장 시작...")
        
        # 데이터셋 로드 (train 셋 사용)
        dataset = FinanceLawMergeRagDataset(split="train")
        ...
        # 전체 데이터셋 저장 (JSON 형태로)
        full_dataset_file = dataset.save_full_dataset(output_format="json")
        print(f"[SUCCESS] 전체 데이터셋 저장 완료: {full_dataset_file}")
        
        print("\n 데이터 확인:")
        print("   - datasets/samples 폴더: 샘플 데이터")
        print("   - datafile/original/HJUNN 폴더: 전체 데이터셋")
