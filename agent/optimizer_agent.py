...
class OptimizerLLM(dspy.Signature):
    ...
    # 구조화된 피드백: [History] + [Feedback] + [Action] 섹션으로 체계적 구성  
    verbal_feedback = dspy.InputField(desc="구조화된 평가 피드백 (현재 성능, 상세 평가, 실패 사례 분석, 개선 방향 포함)")
    # 실패 케이스 요약: verbal_feedback에서 추출한 핵심 실패 정보
    fail_case_feedback = dspy.InputField(desc="실패 사례 요약 (질문, 정답, 문제점, 점수)")
...
OptimizerLLM.__doc__ = OPTIMIZER_SYSTEM_PROMPT  # OPTIMIZER_SYSTEM_PROMPT 사용
...
class PromptOptimizerAgent:
    def __init__(self):
        ...
        # OptimizerLLM 관련 정보 저장용
        self.last_system_prompt = OPTIMIZER_SYSTEM_PROMPT
        self.last_total_input = None
    ...
    def act(self, state):
        ...
        # [디버깅] 구조화된 피드백 확인
        print(f"    [VERBOSE] Structured verbal_feedback preview:")
        print(f"    {verbal_feedback[:500]}...")  # 처음 500자만 미리보기
        ...
        try:
            with dspy.context(lm=self.optimizer_lm):
                ...
                response = self.llm_module(
                    attempted_instruction=attempted_instruction,  
                ...
                # OptimizerLLM이 받은 내용을 DSPy 형식으로 추정하여 저장
                # (실제 캡처는 DSPy 내부 API 복잡성으로 인해 생략)
                self.last_total_input = f"""[DSPy OptimizerLLM Input - Estimated Format]
...
=== EXPECTED OUTPUT ===
new_instruction: [Generated by OptimizerLLM]
"""
                
                print(f"    [DEBUG] Saved estimated OptimizerLLM input (length: {len(self.last_total_input)})")
